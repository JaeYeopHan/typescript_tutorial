...
<< [1. Basic Types](https://jaeyeophan.github.io/2017/04/23/TS-1-Basic-Types/)

이번 포스팅에서는 TypeScript에서의 **클래스(class)** 에 대해 다뤄보겠습니다. 자바스크립트 개발자라고 하더라도 ES6덕분에 우리는 클래스에 이미 많이 익숙해져 있습니다. 그러나 ES6의 클래스는 뭔가가 조금 아쉬웠는데요, 이 부분을 TypeScript가 채워줄 예정입니다. 일단 ES6에서 클래스를 살펴봅니다.
...
```
익숙한 클래스의 형태라고 생각합니다. 익숙하지 않으시다면 [ES6. Class](https://jaeyeophan.github.io/2017/04/18/ES6-6-Class-sugar-syntax/)에 대한 설명을 참고해주세요.
ES6 코드와 비교해봤을 때, 이것 저것 키워드가 많이 추가가 됬는데요, 하나씩 알아보기로 합시다.
...
```
> filed member로 미리 정의되지 않은 인자를 constructor에서 받을 수 없습니다.

ES6에서는 constructor로 받은 인자를 해당 클래스에 멤버로 등록하기 위해 this로 등록을 해줬습니다. 그 제한 또한 없었습니다. 하지만 타입스크립트에서 해당 클래스의 프로퍼티로 등록되지 않은 속성에 대해 constructor에서 받을 수 없습니다.

</br>

### 2. extend를 통한 상속
TypeScript에서도 ES6와 마찬가지 방식으로 상속을 구현할 수 있습니다. `public`, `protected` 접근제어자로 정의된 프로퍼티와 메소드에 접근할 수 있습니다.
```ts
class Person {
  name: string;
...
#### private
`private` 접근 제어자로 정의된 프로퍼티는 클래스 밖에서 접근할 수 없습니다. 이번 예시에서는 `getter`, `setter`를 추가해주기 위해 프로퍼티 명 앞에 `_`(언더바)를 추가했습니다.
```ts
class Person {
  name: string;
  private _job: string;
...
const p1 = new Person("jbee", "Developer");
...
const devPerson = new Developer(true);
...
const p1 = new Person(true); // Error!
const devPerson = new Developer(); // OK!
```
